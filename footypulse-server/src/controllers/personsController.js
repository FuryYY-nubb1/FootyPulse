...
exports.getStats = asyncHandler(async (req, res) => {
  ...
  let statsQuery = `
    SELECT
      ...
      COALESCE(SUM(CASE WHEN mp.minute_out IS NOT NULL AND mp.minute_in IS NOT NULL
        THEN mp.minute_out - mp.minute_in WHEN mp.is_starter THEN 90 ELSE 30 END), 0) AS minutes_played,
  ...
  // Goals
  let goalsQuery = `SELECT COUNT(*) AS goals FROM match_events me JOIN matches m ON me.match_id = m.match_id
    WHERE me.person_id = $1 AND me.event_type IN ('goal', 'penalty')`;
  const goalsValues = [id];
  if (seasonId) { goalsQuery += ` AND m.season_id = $2`; goalsValues.push(seasonId); }
  const goalsResult = await db.query(goalsQuery, goalsValues);

  // Assists
  let assistsQuery = `SELECT COUNT(*) AS assists FROM match_events me JOIN matches m ON me.match_id = m.match_id
    WHERE me.related_person_id = $1 AND me.event_type IN ('goal', 'penalty')`;
  const assistsValues = [id];
  if (seasonId) { assistsQuery += ` AND m.season_id = $2`; assistsValues.push(seasonId); }
  ...
  let cardsQuery = `SELECT
    ...
    FROM match_events me JOIN matches m ON me.match_id = m.match_id WHERE me.person_id = $1`;
  ...
  let advancedQuery = `SELECT
    ...
    FROM match_players mp JOIN matches m ON mp.match_id = m.match_id WHERE mp.person_id = $1`;
  const advValues = [id];
  if (seasonId) { advancedQuery += ` AND m.season_id = $2`; advValues.push(seasonId); }
...
exports.getAchievements = asyncHandler(async (req, res) => {
  ...
  const result = await db.query(
    `SELECT a.*, comp.name AS competition_name, s.name AS season_name
     ...
     WHERE a.person_id = $1
     ORDER BY a.year DESC, a.is_major DESC`, [id]
  );
...
exports.getTransfers = asyncHandler(async (req, res) => {
  ...
  const result = await db.query(
    ...
    tt.name AS to_team_name, tt.logo_url AS to_team_logo
     ...
     ORDER BY tr.transfer_date DESC NULLS LAST`, [id]
  );
  res.json({ success: true, data: result.rows });
});
...
exports.getTopValued = asyncHandler(async (req, res) => {
  ...
  const players = await PersonModel.getTopValuedByPosition(limit);
