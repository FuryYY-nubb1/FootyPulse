...
exports.getMatches = asyncHandler(async (req, res) => {
  ...
  let query = `
    ...
    s.name AS season_name, comp.name AS competition_name, comp.short_name AS competition_short,
    comp.logo_url AS competition_logo, st.name AS stadium_name
    FROM matches m
...
exports.getTransfers = asyncHandler(async (req, res) => {
  ...
  const result = await db.query(
    `SELECT tr.*, p.display_name AS player_name, p.photo_url AS player_photo, p.primary_position,
     ...
     ORDER BY tr.transfer_date DESC NULLS LAST LIMIT $2`,
    [id, limitVal]
  );
  res.json({ success: true, data: result.rows });
});
...
exports.getSquadValueRanking = asyncHandler(async (req, res) => {
  ...
  const ranking = await TeamModel.getSquadValueRanking(limit);
